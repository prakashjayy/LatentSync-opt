...
- **ffmpeg** – Install via your package manager (e.g., `apt install ffmpeg`, `brew install ffmpeg`, or `conda install -c conda-forge ffmpeg`)

### Quick Setup

Run the setup script to create a virtual environment, install dependencies, and download checkpoints:
...
- **With uv** (recommended – no need to activate the environment):
  ```bash
...
- **Linux/Windows (CUDA)**: PyTorch with CUDA 12.1 is installed from the PyTorch index. This is the primary supported platform.
- **macOS**: CPU-only PyTorch is used (CUDA builds are not available). Note: `decord` has no wheels for macOS ARM (Apple Silicon)—the project is primarily tested on Linux.
